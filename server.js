...
const payments = createPayments(reportStore);
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && ['/api/checkout','/api/checkout/status','/api/stripe/webhook'].includes(url.pathname)) {
    ...
    try {
      ...
      return json(res,200,url.pathname==='/api/checkout' ? await payments.checkout(body.scanId) : await payments.fulfill(body.sessionId));
  ...
  if (req.method === 'GET' && /^\/api\/scan\/[^/]+$/.test(url.pathname)) {
    ...
    return json(res, 200, { status: job?.status || 'done', step: job?.step || 'Complete', result: freeReport(job?.result || saved?.result), error: job?.error, ms: job?.ms, checkoutAvailable:payments.ready, reportPreviewAvailable:demoAllowed(req) });
  }
