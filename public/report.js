...
function renderReport(r) {
  ...
  return `<div id="paid-preview" class="free-report">
    ...
    <div class="free-upgrade"><div><h2>Unlock the complete intelligence layer</h2><p>Explore the evidence and recommendations behind your score.</p></div><button type="button" data-view="full">View premium report →</button></div>
  </div>
  <div id="full-detail" class="hidden" tabindex="-1"><div class="report-actions"><button type="button" data-view="free">← Free report</button><button type="button" data-print>Download PDF</button></div><div class="preview-notice" tabindex="-1"><b>Full report preview</b><span>Automated scan findings · Demo access · No payment collected</span></div><article class="report-document">${renderDetailedReport(r)}<section class="fix-roadmap"><div class="sechead"><h3>Fix Roadmap</h3></div><p class="muted">Recommendations from this scan. Effort and timing require implementation review.</p>${fixes.length ? `<div class="benchmark-scroll"><table><thead><tr><th>Recommended task</th><th>Current score</th><th>Implementation</th></tr></thead><tbody>${fixes.map(c => `<tr><td>${esc(c.fix.headline)}</td><td>${c.value}/100</td><td>${esc(c.fix.where || 'Review check details')}</td></tr>`).join('')}</tbody></table></div>` : '<p>No implementation recommendations are available for this scan.</p>'}</section></article><div class="report-actions"><button type="button" data-view="free">← Free report</button><button type="button" data-print>Download PDF</button></div></div>`;
}
